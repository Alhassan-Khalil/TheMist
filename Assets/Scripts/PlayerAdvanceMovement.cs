...
public class PlayerAdvanceMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enableZoom)
        {
            ...
            if (zoomKey && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                {
                    isZoomed = true;
        ...
        if (enableJump && isJumpingInput && isGrounded)
        {
            Jump();
        }
        ...
        if (enableCrouch)
        {
            ...
            else if (isCrouchInput && holdToCrouch)
            {
                //TODO : what is this ?
                isCrouched = true;
                Crouch();
...
// Custom Editor
#if UNITY_EDITOR
[CustomEditor(typeof(PlayerAdvanceMovement)), InitializeOnLoadAttribute]
public class PlayerAdvanceMovementEditor : Editor
{
    PlayerAdvanceMovement fpc;
    SerializedObject SerFPC;

    private void OnEnable()
    {
        fpc = (PlayerAdvanceMovement)target;
